...
void Test_CircularBufferCreate(void)
{
	...
	UCUNTI_CheckIsNotNull(buffer);
	UCUNIT_CheckIsNull(buffer->buffer_start);

	/* Init Circular Buffer with size of 4 */
	CircBufInit(buffer, 4);
	CheckIsNotNull(buffer->buffer_start);
...
void Testsuite_BasicBufferTest(void)
{
	CircularBuffer_t * buffer = CircBufCreate();
